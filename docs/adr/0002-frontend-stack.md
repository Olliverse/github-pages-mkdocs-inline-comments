...
- **esbuild as the only build tool**, source in `frontend/`, output bundled to `src/mkdocs_inline_comments/assets/ghc.js` + `ghc.css`.
- **Zero runtime npm dependencies.** TextQuoteSelector anchoring is an own minimal implementation modeled on Hypothesis' MIT-licensed `dom-anchor-text-quote`; fuzzy matching is deliberately omitted — anchor orphans are a designed state ([QDR-001](../qdr/0001-orphaned-annotations.md)) — keeping the dependency count at zero.
